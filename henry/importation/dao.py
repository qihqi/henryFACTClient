...
class Purchase(SerializableDB[NPurchase]):
    ...
    def deserialize(cls, thedict):
        ...
        if thedict.get('total_gross_weight_kg', None):
            result.total_gross_weight_kg = Decimal(
                result.total_gross_weight_kg)
...
def get_purchase_item_full(dbapi, uid):
    return _get_purchase_item_full_filter(
        dbapi, NPurchaseItem.purchase_id == uid)


def get_purchase_item_full_by_custom(dbapi, uid):
    return _get_purchase_item_full_filter(
        dbapi, NPurchaseItem.custom_item_uid == uid)
...
class CustomItemFull(SerializableData):
    ...
    purchase_item: List[PurchaseItemFull] = dataclasses.field(
...
def get_custom_items_full(dbapi, uid):
    ...
    items = {
        x.uid: CustomItemFull(x) for x in dbapi.search(
            CustomItem,
            purchase_id=uid)}
    for i in item_full:
