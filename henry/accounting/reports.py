...
def generate_daily_report(dbapi, day) -> DailyReport:
    ...
    deleted, other = split_records_binary(
        all_sale, lambda x: x.status == Status.DELETED)
    cashed, noncash = split_records_binary(
        other, lambda x: x.payment_format == PaymentFormat.CASH)
    sale_by_store = group_by_records(
        cashed, attrgetter('almacen_name'), attrgetter('total'))
    ...
    query = dbapi.db_session.query(NPayment).filter(
            deleted != False).filter(NPayment.note_id.in_(ids))  # noqa:712

    # only retension for cash invoices need to be accounted separately.
    by_retension = split_records(
        query, lambda x: x.type == 'retension' and x.note_id in cashids)
    other_cash = sum(
        (x.value for x in by_retension[False] if x.type == PaymentFormat.CASH))
    ...
    check_ids = [x.uid for x in by_retension[False]
                 if x.type == PaymentFormat.CHECK]
    checks = dbapi.db_session.query(NCheck).filter(
        NCheck.payment_id.in_(check_ids))
...
def get_turned_in_cash(dbapi, start_date, end_date, imageserver):
    all_acct = list(dbapi.search(AccountTransaction, **
                    {'date-gte': start_date, 'date-lte': end_date}))
    imgs = {x.objid: x for x in dbapi.search(
        Image, objtype='account_transaction')}
...
def get_transactions(dbapi, paymentapi, invapi, imageserver, start_date, end_date):
    ...
    sales = list(get_sales_as_transactions(
        dbapi, start_date, end_date + delta))
    spent = list(get_spent_as_transactions(dbapi, start_date, end_date))
    turned_in = list(get_turned_in_cash(
        dbapi, start_date, end_date, imageserver))
    payments, payments_credit = get_payments_as_transactions(
        dbapi, start_date, end_date + delta)
...
def get_sale_report_full(invapi, start, end) -> SaleReport:
    invs = invapi.search_metadata_by_date_range(
        start, end, status=Status.COMITTED)
    report = SaleReport()
    ...
    for inv in invs:
        ...
        if inv.almacen_id == 2:
            report.mayor[datestr] += Decimal(inv.subtotal -
                                             (inv.discount or 0)) / 100
        if inv.almacen_id in (1, 3):
            report.menor[datestr] += Decimal(inv.subtotal -
                                             (inv.discount or 0)) / 100
        ...
        for item in inv_full.items:
            ...
            prod_sale_map[cod].cant += item.cant * \
                (item.prod.multiplicador or 1)
            prod_sale_map[cod].value += (item.cant * Decimal(
                item.prod.precio2 or item.prod.precio1) / 100)
